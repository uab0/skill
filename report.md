...
此 track 不依賴外部套件，只用 Python standard library。LLM 只在低信心語意判斷時提供 bounded `candidate_plan`；最後欄位檢查、missing value 處理、數值計算與 file-based contract 都由 scripts 控制。metric 也設計成可抗 perturbation：改 task_id、重排 rows、加入無關欄位、同義詞與 selected-column missing values 都應可驗證。
...
- Open Track 可增加小型多步能力，例如 group aggregate 後取 top group 或先分組再比較比例；但需先定義 deterministic metric 與 perturbation tests。
